...
| 风险 | 逻辑为何可能失效 | 最快证伪 | 判断敏感性 | DD 优先级 |
| --- | --- | --- | --- | --- |
| 性能提升不可复现 | 若效果只存在于精选环境，技术差异化不成立 | 在代表性任务和相同基线下复现一次 | 失败将实质削弱产品逻辑；成功也只能缩小技术不确定性 | 优先验证 |
| 客户转化弱于 pipeline | 若意向不能转为持续付费使用，预算拉动逻辑不成立 | 抽样勾稽合同、使用、收入确认和回款 | 低转化或非持续使用将实质削弱商业判断 | 优先验证 |
| 交付依赖重定制 | 若工时随客户线性增长，软件规模化逻辑被削弱 | 核验客户级部署工时与可复用组件 | 重定制将改变毛利与可扩展性判断 | 重点跟进 |

...
很多公司研究并不是找不到信息，而是材料、证据、判断和更新混在了一起：公司口径被写成事实，结论缺少真正的反方与证伪，尽调问题很多却未必改变判断，新材料一来又覆盖了旧判断。AI 可以把报告写得很完整，却未必让它更可追溯、更容易复核。

这套仓库把这些问题拆成六个可复用的 Skills：先判断材料能支持什么，再形成可被推翻的候选判断，把风险转成真正会改变判断的尽调，并在新信息到来时区分材料变化、证据变化和判断变化。每个 Skill 都有明确的输入要求、输出结构、证据语义和质量检查，因此它不是普通 Prompt Collection。
